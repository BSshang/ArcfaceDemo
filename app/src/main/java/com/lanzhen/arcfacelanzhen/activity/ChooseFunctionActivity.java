...
import com.lanzhen.arcfacelanzhen.fragment.ChooseDetectDegreeDialog;
import com.lanzhen.arcfacelanzhen.fragment.PermissionDegreeDialog;
...
public class ChooseFunctionActivity extends BaseActivity {
    ...
    private boolean checkSoFile(String[] libraries) {
        ...
        if (dir.exists() && dir.isDirectory()) {
            Log.w(TAG, "NativeLibraryDir exists: " + dir.getAbsolutePath());
            ...
            if (files != null && files.length > 0) {
                ...
                Log.w(TAG, "NativeLibraryDir is empty.");
            }
        } else {
            Log.w(TAG, "NativeLibraryDir does not exist.");
        }
        ...
        File[] files = dir.listFiles();
        if (files == null || files.length == 0) {
            Log.w(TAG, "checkSoFile: files = "+false);
            return false;
        }
